...
RSpec.describe "Pruebas programación funcional gema Huella Nutricional"do
	...
	describe "Calculando la huella nutricional DEL PLATO"do
		...
		it "debe existir el metodo que devuelve la huella nutricional "do
			...
			expect(@plato_entrante.huella_nutricional_plato).to eq 1
	...
	describe "Calculando  huella nutricional del plato y precios "do
		...
		it "Se ha de poder obtener el plato con mayor huella nutricional "do
			...
			expect(@menu1.max).to eq @plato_primero
...
RSpec.describe "Pruebas programación funcional clase DSL plato y menú" do
	...
	context " Probando el dsl de plato " do
		...
		it "La lista de platos se pasa correctamente"do
			expect(@plato_hamburguesa.instance_variable_defined?(:@lista_alimentos)).to eq true
